...
def load_db_registry(domain_key: str) -> dict:
    """
    data/<domain_key>/db_registry.json 로드.
    파일이 없으면 빈 dict 반환.
    """
    current_dir  = os.path.dirname(os.path.abspath(__file__))
    project_root = os.path.dirname(current_dir)
    path = os.path.join(project_root, "data", domain_key, "db_registry.json")
    if os.path.exists(path):
        with open(path, encoding='utf-8') as f:
            return json.load(f)
    return {}
...
if check_password():
    ...
    with st.sidebar:
        ...
        # ── 지식베이스 선택 ───────────────────────────────────────
        # domain_registry의 db_keys 순서를 기준으로,
        # 도메인별 db_registry.json에서 display_name/description을 읽어옴
        db_registry       = load_db_registry(selected_domain_key)
        available_db_keys = list(db_registry.keys())
