...
MEG_ChatBot/
├── src/
│   ├── chatbot.py              # 메인 Streamlit 앱 + RAG 챗봇 엔진
│   ├── preprocess_meg.py       # 원본 Excel → 정제 데이터 전처리 (Windows 전용)
│   └── table_parser.py         # 정제 데이터 → LLM 마크다운 텍스트 변환
│
└── src/preprocess/             # 런타임에 생성되는 전처리 작업 폴더 (git 제외)
    ├── data/                   # 원본 Excel 체크리스트 파일 위치
    ├── converted_csv/          # Excel → CSV 변환 결과
    ├── error/                  # 괄호 포함 파일명 처리용 임시 폴더
    └── result/
...
[원본 Excel 체크리스트]
  ...
  Excel → win32com → CSV 변환
  NO / ITEM / GUIDE 컬럼 추출
        ↓
  preprocessed_data_semi.xlsx  (1차 중간 저장)
...
- **지식베이스**: `prepare_knowledge_base(file_pattern)` — `@st.cache_resource`로 캐싱, `preprocess/result/final_text_data_*.xlsx` 로드
...
**LLM 프롬프트 답변 규칙 (chatbot.py:95-111)**
...
- **`convert_all_excel_to_csv()`**: `src/preprocess/data/` 하위 모든 `.xlsx`를 COM 자동화로 CSV 변환
  - 파일명에 `[`, `]` 포함 시 임시 폴더(`error/`)에 복사 후 처리
  - CSV 경로 길이 218자 초과 시 스킵
  - Excel 초기화 실패 시 최대 3회 재시도
- **`process_and_save_checklists()`**: CSV에서 헤더 행(`NO`, `ITEM`, `GUIDE` 포함) 탐색 후 데이터 추출
  - `NO` 컬럼이 단일 알파벳(`A`~`Z`)인 행만 유효 데이터로 처리
  - ITEM 다중 컬럼 병합, GUIDE 다중 컬럼 병합 (콤마 구분)
  - `FIGURE` 컬럼 이전까지만 GUIDE로 인식
- **`run_2nd_preprocessing()`**: Title 컬럼 정제
...
- **입력**: `preprocess/result/preprocessed_result.xlsx` (Title, Item, Guide, Reason 컬럼)
- **출력**: `final_text_data_{n}.xlsx` (Text 컬럼, 500행 단위 청크)
...
- `Reason` 컬럼은 현재 미사용 (향후 보강 예정, 코드 주석 참고)
...
### 보안
- `chatbot.py:25` — 비밀번호가 소스코드에 하드코딩되어 있음
  ```python
...
- `chatbot.py`와 `table_parser.py`는 크로스플랫폼 동작

### LLM / Ollama
- 로컬에 Ollama 서버가 `http://localhost:11434`에서 실행 중이어야 함
- 사용 모델: `gemma2` (임베딩 + 생성 모두 동일 모델 사용)
- `chatbot.py`의 `collection_name="design_bot_final_v8"` — 버전 변경 시 컬렉션명 업데이트 필요

### 데이터 파일
- `preprocess/result/final_text_data_*.xlsx` 파일이 없으면 앱이 에러를 표시하고 종료
- `table_parser.py`의 입력 파일명이 `preprocessed_result.xlsx`인데, `preprocess_meg.py`의 출력은 `preprocessed_data_final.xlsx`로 **파일명 불일치** 존재 → 확인 및 통일 필요
...
```bash
# Excel 원본을 src/preprocess/data/ 에 위치시킨 후
...
## 향후 개선 예정 사항 (코드 주석 기반)

- `Reason` 컬럼 데이터 보강 후 프롬프트에 설계 근거 포함 (`chatbot.py:103`, `table_parser.py:60-61`)
- 비밀번호 하드코딩 제거
